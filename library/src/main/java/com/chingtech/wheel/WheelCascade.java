...
public class WheelCascade {
	...
	public void initCascadePicker(String[] firstArray,
			final String[][] secondArray) {
		this.firstArray = firstArray;
		this.secondArray = secondArray;
		index = 0;
		index1 = 0;
		firstView = (WheelCityView) view.findViewById(R.id.wheelcascade_first);
		firstView.setCyclic(false);// 不循环
		firstView.setVisibleItems(3);
		firstView.setViewAdapter(new CascadeAdapter(context, firstArray));

		secondView = (WheelCityView) view
				.findViewById(R.id.wheelcascade_second);
		...
		firstView
				.addChangingListener((OnWheelChangedListener) wheelListener_first);
		secondView
				.addChangingListener((OnWheelChangedListener) wheelListener_second);
	 ...
	 */
	private void updateCascade(WheelCityView wheelView, int index,
			String[][] secondArray) {
		ArrayWheelAdapter<String> adapter = new ArrayWheelAdapter<String>(
				context, secondArray[index]);
